...
export class PaymentTransactionService {
  ...
  async createPayment(data: CreatePaymentData) {
    ...
    this.logger.log(
      `Created payment ${payment.id} for order ${orderId}, amount: ${amount}`,
    );
    return payment;
  }
  ...
  private async updateOrderStatusBasedOnPayments(orderId: number) {
    ...
    if (currentOrder && currentOrder.status !== newOrderStatus) {
      ...
      this.logger.log(
        `Updated order ${orderId} status from ${currentOrder.status} to ${newOrderStatus}`,
      );
    }
  ...
  async createRefund(
    ...
    this.logger.log(
      `Created refund payment ${refundPayment.id} for ${refundAmount} from payment ${originalPaymentId}`,
    );
