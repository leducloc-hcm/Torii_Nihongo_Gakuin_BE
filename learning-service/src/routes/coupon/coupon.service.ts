...
export class CouponService {
  ...
  async createCoupon(createDto: CreateCouponDTO, createdBy: number) {
    ...
    if (!createdBy || typeof createdBy !== "number") {
      throw new BadRequestException(
        "Invalid user ID. User must be authenticated.",
      );
    }

    // Validate coupon code uniqueness
    const existingCoupon = await this.couponRepository.checkCouponExists(
      createDto.code,
    );
    if (existingCoupon) {
    ...
    if (createDto.type !== CouponType.GIFT) {
      if (!createDto.discountType || !createDto.discountValue) {
        throw new BadRequestException(
          "Discount type and value are required for discount coupons",
        );
      }
    ...
    const coupon = await this.couponRepository.create({
      ...
      discountValue: createDto.discountValue
        ? Math.floor(createDto.discountValue)
        : null,
      minOrderAmount: createDto.minOrderAmount
        ? Math.floor(createDto.minOrderAmount)
        : null,
      maxDiscountAmount: createDto.maxDiscountAmount
        ? Math.floor(createDto.maxDiscountAmount)
        : null,
    ...
    this.activityLogService.log({
      ...
      metadata: { code: coupon.code, type: coupon.type, title: coupon.title },
    });
  ...
  async getCoupon(id: number) {
    ...
    const totalRedemptions = Object.values(stats).reduce(
  ...
  async updateCoupon(
    ...
    if (updateDto.startsAt || updateDto.endsAt) {
      const startDate = updateDto.startsAt
        ? new Date(updateDto.startsAt)
        : existingCoupon.startsAt;
      const endDate = updateDto.endsAt
        ? new Date(updateDto.endsAt)
        : existingCoupon.endsAt;
    ...
    await this.createAuditLog(
      id,
      updatedBy,
      CouponAuditAction.UPDATED,
      existingCoupon,
      updatedCoupon,
    );

    this.logger.log(
      `Coupon ${existingCoupon.code} updated by user ${updatedBy}`,
    );
  ...
  async submitForApproval(id: number, submittedBy: number) {
    ...
    if (coupon.status !== CouponStatus.DRAFT) {
      throw new BadRequestException(
        "Only draft coupons can be submitted for approval",
      );
    }
    ...
    await this.createAuditLog(
      id,
      submittedBy,
      CouponAuditAction.SUBMITTED_FOR_APPROVAL,
      coupon,
      updatedCoupon,
    );
  ...
  async approveOrRejectCoupon(
    ...
    if (coupon.status !== CouponStatus.PENDING_APPROVAL) {
      throw new BadRequestException(
        "Only pending coupons can be approved or rejected",
      );
    }
    ...
    const auditAction = isApproval
      ? CouponAuditAction.APPROVED
      : CouponAuditAction.REJECTED;
    await this.createAuditLog(id, actionBy, auditAction, coupon, updatedCoupon);

    this.logger.log(
      `Coupon ${coupon.code} ${approvalDto.action}d by user ${actionBy}`,
    );

    this.activityLogService.log({
      userId: actionBy,
      action: isApproval ? "COUPON_APPROVED" : "COUPON_REJECTED",
      entity: "COUPON",
      entityId: id,
  ...
  async toggleCouponStatus(id: number, actionBy: number) {
    ...
    if (
      ...
      throw new BadRequestException(
        "Only approved coupons can be activated/deactivated",
      );
    }
    ...
    this.logger.log(
      `Coupon ${coupon.code} ${newStatus.toLowerCase()} by user ${actionBy}`,
    );
    return updatedCoupon;
  }
  ...
  async validateCoupon(
    validationDto: ValidateCouponDTO,
    userId: number,
    ...
    if (coupon.perUserLimit) {
      const userRedemptions =
        await this.couponRepository.getUserRedemptionCount(coupon.id, userId);
    ...
    if (
      ...
      validationDto.totalAmount < coupon.minOrderAmount
    ) {
      errors.push(
    ...
    const eligibleCoursesInCart = userCourseIds.filter((id) =>
    ...
    if (coupon.type === CouponType.DISCOUNT_MULTI && coupon.requireFullCombo) {
      const requiredCourseIds = coupon.courses
        .filter((cc) => cc.required)
        .map((cc) => cc.courseId);
      const missingRequiredCourses = requiredCourseIds.filter(
        (id) => !userCourseIds.includes(id),
      );
    ...
    if (
      ...
      const eligibleAmount = this.calculateEligibleAmount(
        eligibleCoursesInCart,
        coupon.courses,
      );
      ...
      discount = {
        ...
        maxDiscountReached: coupon.maxDiscountAmount
  ...
  async redeemGiftCoupon(
    ...
    const existingRedemption = await this.couponRepository.findRedemption(
      coupon.id,
      userId,
    );
    ...
    for (const couponCourse of coupon.courses) {
      ...
      const existingEnrollment =
        await this.enrollmentRepository.findByUserAndCourse(userId, course.id);
      ...
      if (existingEnrollment) {
        ...
        if (
          ...
          if (availableClasses.length === 0) {
            throw new BadRequestException(
              `No available classes for course: ${course.title}`,
            );
          }
    ...
    const redemptionStatus =
      pendingClassSelections.length > 0
        ? RedemptionStatus.PARTIALLY_COMPLETED
  ...
  async selectClassForGift(selectDto: SelectClassForGiftDTO, userId: number) {
    ...
    const redemption = await this.couponRepository.findRedemption(
      coupon.id,
      userId,
    );
    if (!redemption) {
    ...
    const classExists = this.validateClassSelection(
      selectDto.courseId,
      selectDto.classId,
    );
    if (!classExists) {
    ...
    // Update redemption
    const updatedPendingClasses = redemption.pendingClassSelections.filter(
      (id) => id !== selectDto.courseId,
    );
    const newStatus =
      updatedPendingClasses.length === 0
        ? RedemptionStatus.COMPLETED
        : RedemptionStatus.PARTIALLY_COMPLETED;
    ...
    await this.couponRepository.updateRedemption(redemption.id, {
      ...
      status: newStatus,
      completedAt:
        newStatus === RedemptionStatus.COMPLETED ? new Date() : undefined,
    });

    this.logger.log(
      `Class ${selectDto.classId} selected for course ${selectDto.courseId} by user ${userId}`,
    );
  ...
  async createPurchasedGiftCoupon(
    userId: number,
    dto: PurchaseGiftCouponDTO,
  ): Promise<GiftPurchaseResponseDTO> {
    // Validate courses exist
    ...
    return {
      ...
      purchasedAt:
        updatedCoupon.purchasedAt?.toISOString() || new Date().toISOString(),
      totalAmount,
  ...
  async activateGiftCouponAfterPayment(
    couponId: number,
    orderId: number,
  ): Promise<void> {
