...
export class QuestionRepository {
  ...
  async isQuestionUsed(questionId: number): Promise<boolean> {
    ...
    // Note: assessmentAnswer is in assessment schema, not learning schema
    const [quizAnswersCount] = await Promise.all([
      this.prisma.quizAnswer.count({
        where: { questionId },
      }),
    ])
    const assessmentAnswersCount = 0 // Assessment answers are in assessment-service
