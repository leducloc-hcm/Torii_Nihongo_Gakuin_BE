...
export class QuestionController {
  ...
  async cloneQuestion(@Param('id', ParseIntPipe) id: number, @Body() cloneDto: CloneQuestionDTO) {
    ...
    const modifications: any = cloneDto.options ? {
      ...cloneDto,
      options: cloneDto.options.filter((opt): opt is { content: string; isCorrect: boolean; order: number; mediaId?: number } => 
        opt.content !== undefined && opt.isCorrect !== undefined && opt.order !== undefined
      ).map(opt => ({
        content: opt.content!,
    ...
    return this.questionService.cloneQuestion(id, modifications)
  }
