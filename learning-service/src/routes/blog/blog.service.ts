...
import { CreateBlogInput, UpdateBlogInput, QueryBlogsInput } from "./blog.dto";
import {
  BlogWithRelations,
  BlogWhereInput,
  BlogOrderByInput,
...
export class BlogService {
  ...
  async create(
    ...
    if (tagIds && tagIds.length > 0) {
      const { exists, missingIds } =
        await this.blogRepository.checkTagsExist(tagIds);
      if (!exists) {
        throw new BadRequestException(
          `Tags with IDs [${missingIds.join(", ")}] do not exist`,
        );
      }
    }
  ...
  async findAll(queryDto: QueryBlogsInput) {
    const { page, limit, search, tagId, authorId, sortBy, sortOrder } =
      queryDto;
  ...
  async update(
    ...
    if (updateBlogDto.slug && updateBlogDto.slug !== existingBlog.slug) {
      const slugExists = await this.blogRepository.checkSlugExists(
        updateBlogDto.slug,
        id,
      );
      if (slugExists) {
    ...
    if (updateBlogDto.tagIds && updateBlogDto.tagIds.length > 0) {
      const { exists, missingIds } = await this.blogRepository.checkTagsExist(
  ...
  async findByAuthor(authorId: number, queryDto: QueryBlogsInput) {
    return this.findAll({ ...queryDto, authorId });
  }

  async findByTag(tagId: number, queryDto: QueryBlogsInput) {
