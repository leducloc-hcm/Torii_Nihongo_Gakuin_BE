...
import { Tag, TagResponseType } from './tag.model'

@Injectable()
export class TagService {
  constructor(private readonly tagRepository: TagRepository) {}

  async create(createTagDto: CreateTagDTO): Promise<TagResponseType> {
  ...
  async update(id: number, updateTagDto: UpdateTagDTO): Promise<TagResponseType> {
    // Check if tag exists
    await this.findOne(id)

    // If slug is being updated, check if it's already taken
    if (updateTagDto.slug) {
      const slugExists = await this.tagRepository.checkSlugExists(updateTagDto.slug, id)
      if (slugExists) {
