...
export class OnlineClassController {
  ...
  @ApiResponse({
    status: 200,
    description: "Online classes retrieved successfully",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "My enrolled online class retrieved successfully",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "My assigned online classes retrieved successfully",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Upcoming online class sessions retrieved successfully",
  })
  ...
  async generateJoinToken(
    ...
    try {
      const joinToken = await this.onlineClassService.generateJoinToken(
        classId,
        sessionId,
        userId,
      );
  ...
  @ApiResponse({
    status: 200,
    description: "Online class session started successfully",
  })
  async startOnlineClassSession(
    ...
    try {
      ...
      const session = await this.onlineClassService.startOnlineClassSession(
        sessionId,
        lecturerId,
      );
      ...
      throw new HttpException(
        {
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to start online class session",
        },
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description: "Online class session ended successfully",
  })
  async endOnlineClassSession(
    ...
    try {
      const result = await this.onlineClassService.endOnlineClassSession(
        sessionId,
        userId,
      );
      ...
      throw new HttpException(
        {
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to end online class session",
        },
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  @ApiOperation({
    summary:
      "Webhook endpoint for recording completion notification from Janus server",
  })
  @ApiResponse({
    status: 200,
  ...
  async handleRecordingComplete(
    ...
    try {
      const result = await this.onlineClassService.updateRecordingUrl(
        body.janusRoomId,
        body.recordingUrl,
      );
