...
export class OnlineClassService {
    ...
    try {
      ...
      if (!activeSession) {
        throw new BadRequestException(
          "No active session available for this class",
        );
      }
      ...
      if (activeSession.janusRoomId) {
        try {
          this.logger.log(
            `🎯 GenerateJoinToken: Querying participants for room ${activeSession.janusRoomId}`,
          );

          // Create a temporary session to query participants
          const tempSessionId = await this.janusService.createSession();
          if (tempSessionId) {
            const tempHandleId = await this.janusService.attachPlugin(
              tempSessionId,
            ...
            if (tempHandleId) {
              ...
              this.logger.warn(
                "⚠️ GenerateJoinToken: Failed to attach plugin for participants query",
              );
            }
          } else {
            this.logger.warn(
              "⚠️ GenerateJoinToken: Failed to create temp session for participants query",
            );
          }
        } catch (error) {
          this.logger.error(
            "❌ GenerateJoinToken: Failed to get participants from Janus:",
            error,
          );
        ...
        this.logger.log(
          "ℹ️ GenerateJoinToken: No Janus room ID available, skipping participants query",
        );
      }
      ...
      const iceServers = [
        {
          urls:
            process.env.JANUS_STUN_URL ||
            "stun:janus.torii-nihongo-gakuin.io.vn:3478",
        },
          ...
          ? [
              {
                urls:
                  process.env.JANUS_TURN_URL ||
                  "turn:janus.torii-nihongo-gakuin.io.vn:3478",
      ...
      this.logger.log(
        "🚀 GenerateJoinToken: Returning response with participants:",
        {
          classId,
  ...
  async startOnlineClassSession(
    ...
    try {
      ...
      this.logger.log(
        `Started online class session ${session.id}  with Janus room ${janusRoom.room}`,
      );
    ...
    try {
      ...
      if (activeSession.janusRoomId) {
        try {
          ...
          this.logger.error(
            `Failed to destroy Janus room ${activeSession.janusRoomId}:`,
            error,
          );
        }
      }
  ...
  > {
    try {
      ...
      return participants.map((p) => ({
        ...
        joinedAt: p.joinedAt,
        isActive: !p.leftAt,
        capabilities: this.getUserPermissions(
          p.user.role === Role.LECTURER ? "lecturer" : "customer",
        ),
      }));
    } catch (error) {
  ...
  async getJanusParticipants(classId: string): Promise<{
    ...
    try {
      ...
      try {
        this.logger.log(
          `🔍 Querying participants for Janus room: ${activeSession.janusRoomId}`,
        );
        ...
        if (tempSessionId) {
          ...
          if (tempHandleId) {
            ...
            this.logger.log(
              `👥 Raw Janus participants response:`,
              JSON.stringify(janusParticipants, null, 2),
            );
            ...
            this.logger.warn(
              "⚠️ Failed to attach plugin - no handle ID received",
            );
          }
        } else {
          this.logger.warn(
            "⚠️ Failed to create temp session for participants query",
          );
        }
      } catch (error) {
  ...
  async startRecording(
    ...
    if (onlineClass.lecturerId !== lecturerId) {
      throw new BadRequestException(
        "Only the class lecturer can start recording",
      );
    }
    ...
    const filename =
      options.filename || `class_${classId}_${new Date().toISOString()}.webm`;
  ...
  async stopRecording(classId: string, lecturerId: number): Promise<any> {
    ...
    if (onlineClass.lecturerId !== lecturerId) {
      throw new BadRequestException(
        "Only the class lecturer can stop recording",
      );
    }
    ...
    return {
      ...
      stoppedAt,
      message:
        "Recording stopped. Upload the recording data to complete the process.",
    };
  }
  ...
  async getRecordingUploadUrl(
    ...
    if (onlineClass.lecturerId !== lecturerId) {
      throw new BadRequestException(
        "Only the class lecturer can upload recordings",
      );
    }
  ...
  async confirmRecordingUpload(
    ...
    if (onlineClass.lecturerId !== lecturerId) {
      throw new BadRequestException(
        "Only the class lecturer can confirm recordings",
      );
    }

    this.logger.log(
      `Recording confirmed for class ${classId}: ${data.recordingUrl}`,
    );
  ...
  async updateRecordingUrl(
    ...
    this.logger.log(
      `📹 Updating recording URL for Janus room ${janusRoomId}: ${recordingUrl}`,
    );
    ...
    if (!liveSession) {
      this.logger.error(
        `❌ Live session not found for Janus room ${janusRoomId}`,
      );
   ...
   */
  private async notifyRecordingAvailable(
    classId: number,
    recordingUrl: string,
  ): Promise<void> {
    try {
  ...
  shareDocument(
    classId: string,
    lecturerId: number,
    document: ShareDocumentDto,
  ): any {
    // Implementation for sharing documents
    throw new BadRequestException(
      "Document sharing functionality not implemented yet",
    );
  }
  ...
  performParticipantAction(
    ...
    throw new BadRequestException(
      "Participant control functionality not implemented yet",
    );
  }
  ...
  getClassAnalytics(classId: string, lecturerId: number): ClassAnalyticsDto {
    ...
    throw new BadRequestException(
      "Analytics functionality not implemented yet",
    );
  }
  async getMyEnrolledOnlineClass(userId: number) {
    try {
      ...
      throw new BadRequestException(
        "Failed to retrieve enrolled online classes",
      );
    }
  }
  async getMyAssignedOnlineClasses(userId: number) {
    try {
      ...
      throw new BadRequestException(
        "Failed to retrieve assigned online classes",
      );
    }
  }
  ...
  async getUpcomingOnlineClassSessions(userId: number) {
    try {
      ...
      throw new BadRequestException(
        "Failed to retrieve upcoming online class sessions",
      );
    }
  }
}
