...
export class FlashcardService {
  ...
  async getCardById(id: number, userId?: number) {
    ...
    if (userId !== undefined && card.deck.ownerId !== userId) {
      const deck = await this.flashcardRepository.findDeckById(card.deck.id, userId)
      if (!deck) {
  ...
  async submitStudyResponse(userId: number, response: StudyResponseInput) {
    ...
    const progress = await this.flashcardRepository.findOrCreateCardProgress(userId, response.cardId)

    // Calculate new values using SuperMemo-2 algorithm
    const newProgress = this.calculateSuperMemo2(progress.ef, progress.interval, progress.repetitions, response.grade)

    // Update progress
    return await this.flashcardRepository.updateCardProgress(userId, response.cardId, {
      ...newProgress,
      lastGrade: response.grade,
    })
  }

  async submitBulkStudyResponse(userId: number, responses: BulkStudyResponseInput) {
    const results: Array<{
      success: boolean
      cardId: number
      progress?: any
    ...
    for (const response of responses.responses) {
      try {
        ...
        results.push({ success: true, cardId: response.cardId, progress: result })
      } catch (error: any) {
        results.push({
          success: false,
  ...
  async getDeckStatistics(deckId: number, userId: number) {
    ...
    const cards = await this.flashcardRepository.findDeckCards(deckId, userId)
    const progress = await this.flashcardRepository.findUserCardProgress(userId, deckId)
    ...
    const stats = {
      ...
      dueCards: progress.filter((p) => p.dueAt <= now && p.repetitions > 0).length,
      masteredCards: progress.filter((p) => p.repetitions >= 5 && p.ef >= 2.5).length,
      averageEF:
        progress.length > 0 ? Number((progress.reduce((sum, p) => sum + p.ef, 0) / progress.length).toFixed(2)) : 0,
      deck: {
        id: deck.id,
        title: deck.title,
        level: deck.level,
  ...
  async getUserStatistics(userId: number) {
    const progress = await this.flashcardRepository.findUserCardProgress(userId)
    const now = new Date()
