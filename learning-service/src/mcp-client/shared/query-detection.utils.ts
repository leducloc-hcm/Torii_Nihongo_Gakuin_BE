...
export function detectQueryType(query: string): QueryType {
  ...
  const hasCourseKeyword = courseKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const hasLessonKeyword = lessonKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const hasFlashcardKeyword = flashcardKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const hasAssessmentKeyword = assessmentKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const hasAssessmentHistoryKeyword = assessmentHistoryKeywords.some(
    (keyword) => lowerQuery.includes(keyword),
  );
  const hasEnrollmentKeyword = enrollmentKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  ...
  // --- Grammar detection ---
  const isGrammarQuery =
    lowerQuery.includes("ngữ pháp") ||
    lowerQuery.includes("grammar") ||
    lowerQuery.includes("giải thích") ||
    lowerQuery.includes("explain") ||
    lowerQuery.includes("cách dùng") ||
    lowerQuery.includes("how to use") ||
    lowerQuery.includes("文法") ||
    lowerQuery.includes("てform") ||
    lowerQuery.includes("て form") ||
    lowerQuery.includes("ている") ||
    lowerQuery.includes("てみる") ||
    lowerQuery.includes("てから") ||
    lowerQuery.includes("ている") ||
    lowerQuery.includes("passive") ||
    lowerQuery.includes("causative") ||
    lowerQuery.includes("conditional") ||
    lowerQuery.includes("điều kiện") ||
    lowerQuery.includes("bị động") ||
    lowerQuery.includes("thể") ||
    (/\bが|は|を|に|で|から|まで|より|と|も|だけ|しか|ほど|くらい\b/.test(
      lowerQuery,
    ) &&
      (lowerQuery.includes("nghĩa") ||
        lowerQuery.includes("meaning") ||
        lowerQuery.includes("意味") ||
        lowerQuery.includes("dùng") ||
        lowerQuery.includes("use")));

  // --- Translation detection ---
  const isTranslationQuery =
    lowerQuery.includes("dịch") ||
    lowerQuery.includes("translate") ||
    lowerQuery.includes("translation") ||
    lowerQuery.includes("nghĩa là gì") ||
    lowerQuery.includes("có nghĩa là") ||
    lowerQuery.includes("what does") ||
    lowerQuery.includes("what is") ||
    lowerQuery.includes("means") ||
    lowerQuery.includes("tiếng việt") ||
    lowerQuery.includes("tiếng anh") ||
    lowerQuery.includes("tiếng nhật") ||
    lowerQuery.includes("japanese") ||
    lowerQuery.includes("hiragana") ||
    lowerQuery.includes("katakana") ||
    lowerQuery.includes("kanji") ||
    lowerQuery.includes("ひらがな") ||
    lowerQuery.includes("カタカナ") ||
    lowerQuery.includes("漢字") ||
    (lowerQuery.includes("từ") && lowerQuery.includes("là gì")) ||
    (lowerQuery.includes("câu") && lowerQuery.includes("dịch"));

  // Priority: Grammar > Translation > Flashcard Creation > Flashcard Search > Assessment History > Assessment Search > Blog > Available Courses > Personal Courses/Enrollment > Course > Lesson > General
  // CRITICAL: Grammar and Translation are Sensei-exclusive tools and must be highest priority
  ...
  if (isGrammarQuery) return QueryType.GRAMMAR;
  if (isTranslationQuery) return QueryType.TRANSLATION;
...
export function isRecommendationQuery(query: string): boolean {
  ...
  return recommendKeywords.some((keyword) =>
    query.toLowerCase().includes(keyword),
  );
}
...
export function detectMultiToolRequirement(
  query: string,
): MultiToolRequirement {
  const lowerQuery = query.toLowerCase();
  ...
  const requiresCourse = courseKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const requiresLesson = lessonKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const requiresFlashcard = flashcardKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const requiresAssessment = assessmentKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
  const requiresBlog = blogKeywords.some((keyword) =>
    lowerQuery.includes(keyword),
  );
