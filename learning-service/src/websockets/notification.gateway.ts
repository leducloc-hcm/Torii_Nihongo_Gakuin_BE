  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  sendPaymentNotification(
    ...
    this.logger.log(
      `Sent payment notification to user ${userId}: ${notification.paymentStatus}`,
    );
  }
  ...
  // Notify a user that a specific device has been revoked
  notifyDeviceRevoked(userId: number, deviceId: number) {
    this.server.to(`user_${userId}`).emit("device-revoked", {
      deviceId,
      message: "Your account has been logged in from another place",
      timestamp: Date.now(),
    });
    this.logger.log(
      `Sent device-revoked notification to user ${userId}, device ${deviceId}`,
    );
  }
  ...
  notifyPaymentPending(
    ...
    this.sendNotification(userId.toString(), {
      ...
      message:
        paymentData.message ||
        "Đang chờ thanh toán. Vui lòng quét mã QR để hoàn tất.",
      data: {
  ...
  notifyEnrollmentCreated(
    ...
    this.logger.log(
      `Sent enrollment notification to user ${userId}, course ${enrollmentData.courseId}`,
    );
  }
