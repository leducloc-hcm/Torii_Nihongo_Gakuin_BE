...
import {
  JanusJoinRoomRequest,
  JanusPublishRequest,
  JanusSubscribeRequest,
} from "../dto/janus.interface";
...
export class JanusService implements OnModuleDestroy {
  ...
  async createSession(): Promise<number | null> {
    ...
    while (retries < maxRetries) {
      try {
        ...
        retries++;
        this.logger.error(
          `Error creating Janus session (attempt ${retries}/${maxRetries}):`,
          error,
        );
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (response.janus === "success" && response.data?.id) {
          ...
          this.logger.log(
            `Attached plugin ${pluginName} to session ${sessionId}, handle: ${handleId}`,
          );
          return handleId;
        }
        ...
        retries++;
        this.logger.error(
          `Error attaching plugin ${pluginName} (attempt ${retries}/${maxRetries}):`,
          error,
        );

        if (retries >= maxRetries) {
          this.logger.error(
            `Max retries reached for attaching plugin ${pluginName}`,
          );
          return null;
        }
  ...
  async createRoom(options: {
    ...
    try {
      ...
      const handleId = await this.attachPlugin(
        sessionId,
  ...
  async destroyRoom(roomId: number): Promise<boolean> {
    try {
      ...
      if (
        destroyResponse.janus === "success" ||
        destroyResponse.plugindata?.data?.videoroom === "destroyed"
      ) {
  ...
  async listParticipants(
    ...
    try {
      ...
      if (
        ...
        response.plugindata?.data?.participants
      ) {
