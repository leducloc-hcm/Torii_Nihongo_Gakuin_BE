...
export class CartController {
  ...
  async getCart(@ActiveUser('userId') userId: number): Promise<CartResponseDTO | null> {
    return this.cartService.getCart(userId)
  }

  @Post('items')
  @HttpCode(HttpStatus.CREATED)
  async addToCart(@ActiveUser('userId') userId: number, @Body() addToCartDto: AddToCartDTO): Promise<CartResponseDTO> {
